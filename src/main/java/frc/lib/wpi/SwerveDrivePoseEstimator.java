      ...
      try {
        m_visionCorrect.accept(
            ...
            StateSpaceUtil.poseTo3dVector(
                getEstimatedPosition().transformBy(visionRobotPoseMeters.minus(sample))));
      } catch (Exception e) {
        Logger.tag("Swerve Pose Est").warn(e);
        m_observer.reset();
        m_observer.setXhat(StateSpaceUtil.poseTo3dVector(sample));
      }
    } else {
      Logger.tag("Swerve Pose Est").warn("Invalid timestamp");
    }
  }
